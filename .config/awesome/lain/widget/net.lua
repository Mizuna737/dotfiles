...
local function factory(args)
	...
	-- Add rolling history
	local history = {}
	local history_limit = math.floor(10 / timeout)
	...
	function net.update()
		...
		for _, dev in ipairs(net.iface) do
			...
			current.sent = current.sent + dev_now.sent
			current.received = current.received + dev_now.received
		...
		for _, sample in ipairs(history) do
			net.net_now.sent = net.net_now.sent + sample.sent
			net.net_now.received = net.net_now.received + sample.received
		end

		net.net_now.sent = string.format(format, net.net_now.sent / #history)
		net.net_now.received = string.format(format, net.net_now.received / #history)

		widget = net.widget
		_G.net_now = net.net_now
		settings()
	end
