-- devices/macropad.lua
-- 5-button pad with short press vs. long press.
-- Assume short press => F21..F25, long press => F26..F30.

local gears = require("gears")
local awful = require("awful")
local myFuncs = require("functions")
-- Modifiers:
local modkey = "Mod4" -- Super
local ctrl = "Control"
local altkey = "Mod1"
local shft = "Shift"

local macropad = {}

macropad.globalkeys = gears.table.join(

	-- Button 1 short => Meta+Alt+1 => Load Misc Config
	awful.key({ modkey, altkey }, "7", function()
		myFuncs.loadWorkspaceConfiguration("Misc")
	end, { description = "M1 => load Misc configuration", group = "macropad" }),
	-- Button 1 long => Meta+Alt+Control+1 => Save Misc Config
	awful.key({ modkey, altkey, ctrl }, "7", function()
		myFuncs.saveWorkspaceConfiguration("Misc")
	end, { description = "M1 hold => overwrite Misc configuration", group = "macropad" }),

	-- Button 2 short => Meta+Alt+2 => Load Obsidian Config
	awful.key({ modkey, altkey }, "8", function()
		myFuncs.loadWorkspaceConfiguration("Obsidian")
	end, { description = "M2 => load Obsidian configuration", group = "macropad" }),
	-- Button 2 long => Meta+Alt+Control+2 => Save Obsidian Config
	awful.key({ modkey, altkey, ctrl }, "8", function()
		myFuncs.saveWorkspaceConfiguration("Obsidian")
	end, { description = "M2 hold => overwrite Obsidian configuration", group = "macropad" }),

	-- Button 3 short => Meta+Alt+3 => Load Work Config (unchanged)
	awful.key({ modkey, altkey }, "0", function()
		myFuncs.loadWorkspaceConfiguration("Work")
	end, { description = "M3 => load Work configuration", group = "macropad" }),
	-- Button 3 long => Meta+Alt+Control+3 => Save Work Config (unchanged)
	awful.key({ modkey, altkey, ctrl }, "0", function()
		myFuncs.saveWorkspaceConfiguration("Work")
	end, { description = "M3 hold => overwrite Work configuration", group = "macropad" }),

	-- Button 4 short => Meta+Alt+4 => Load Code Config
	awful.key({ modkey, altkey }, "-", function()
		myFuncs.loadWorkspaceConfiguration("Code")
	end, { description = "M4 => load Code configuration", group = "macropad" }),
	-- Button 4 long => Meta+Alt+Control+4 => Save Code Config
	awful.key({ modkey, altkey, ctrl }, "-", function()
		myFuncs.saveWorkspaceConfiguration("Code")
	end, { description = "M4 hold => overwrite Code configuration", group = "macropad" }),

	-- Button 5 short => Meta+Alt+5 => Load Entertainment Config
	awful.key({ modkey, altkey }, "=", function()
		myFuncs.loadWorkspaceConfiguration("Entertainment")
	end, { description = "M5 => load Entertainment configuration", group = "macropad" }),
	-- Button 5 long => Meta+Alt+Control+5 => Save Entertainment Config
	awful.key({ modkey, altkey, ctrl }, "=", function()
		myFuncs.saveWorkspaceConfiguration("Entertainment")
	end, { description = "M5 hold => overwrite Entertainment configuration", group = "macropad" })
)

return macropad
