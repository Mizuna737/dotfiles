...
local function entry()
	return widget({ type = "entry" })
end
local function eventbox()
	return widget({ type = "eventbox" })
end
local function hbox()
	return widget({ type = "hbox" })
end
local function label()
	return widget({ type = "label" })
end
local function notebook()
	return widget({ type = "notebook" })
end
local function vbox()
	return widget({ type = "vbox" })
end
local function overlay()
	return widget({ type = "overlay" })
end
...
local init_funcs = {
	...
	set_window_icon = function(w)
		local path = (luakit.dev_paths and os.exists("./extras/luakit.png"))
...
_M.methods = {
	...
	attach_tab = function(w, view, switch, order)
		...
		if not order and taborder then
			order = (switch == false and taborder.default_bg) or taborder.default
		end
	...
	can_quit = function(w)
		...
		if emsg then
			...
			w:error(string.format("Can't close luakit: %s (force close " .. "with :q! or :wq!)", emsg))
			return false
		else
			return true
		end
	...
	search_open_navigate = function(w, view, arg)
		...
		if type(arg) == "widget" then
			assert(arg.type == "webview")
		end
	...
	search_open = function(_, arg)
		...
		if not arg or arg:match("^%s*$") then
			return settings.get_setting("window.new_tab_page")
		end

		arg = lstring.strip(arg)

		-- Handle JS and file URI before splitting arg
		if arg:find("^javascript:") then
			return arg
		end
...
_M.indexes = {
	...
	function(w, k)
		return k == "tabs" and w_priv[w].tabs or nil
	end,
}
...
function _M.new(args)
	...
	setmetatable(w, {
		...
		__newindex = function(_, k, v)
			if k == "tabs" then
				return set_window_notebook(w, v)
			end
			rawset(w, k, v)
		end,
	})
...
settings.register_settings({
	...
	["window.default_search_engine"] = {
		...
		default = "default",
		validator = function(v)
			return settings.get_setting("window.search_engines")[v]
		end,
		desc = [=[
...
if vulnerable_luakit then
	luakit.idle_add(function()
		msg.warn(
			[[your version of WebKit (%s) is outdated and vulnerable!
See https://webkitgtk.org/security/WSA-2017-0009.html for details.]],
