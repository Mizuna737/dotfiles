...
class ContinuousTrigger:
    ...
    activeWhile: str | None # pose name that must be held to activate this trigger; None = no same-hand pose requirement
...
class Binding:
    ...
    requireLeft:  str | None = None  # left hand must hold this pose (cross-hand condition)
    requireRight: str | None = None  # right hand must hold this pose (cross-hand condition)
...
def parsePose(d):
    ...
    return PoseDefinition(
        name=d["name"],
        thumb=d.get("thumb"), index=d.get("index"), middle=d.get("middle"),
        ring=d.get("ring"),   pinky=d.get("pinky"),
    )
...
def measureAllMetrics(landmarks):
    ...
    return {m: measureMetric(landmarks, m)
...
class HandProcessor:
    ...
    def update(self, landmarks, mpLabel, timestampMs):
        ...
        return HandFrameResult(
            pose=pose, swipe=swipe, metrics=metrics,
            rawPose=rawPose, isMoving=self.motionFilter.isMoving(),
        )
...
class ContinuousTracker:
    ...
    def update(self, pose, metrics, timestampMs, enabled=True):
        shouldBeActive = enabled and (self.trigger.activeWhile is None or pose == self.trigger.activeWhile)
...
def _emptyResult():
    ...
    return HandFrameResult(pose=None, swipe=None, metrics={}, rawPose=None, isMoving=False)
...
class TriggerMatcher:
    ...
    def _isSingleHand(self, binding):
        """True if this binding makes no explicit reference to the other hand.

        ChordTrigger is inherently two-handed. Bindings with requireLeft /
        requireRight are deliberately conditioned on both hands. Everything
        else is a single-hand trigger that should be suppressed while both
        hands are visible, to prevent accidental firing during chord gestures.
        """
        return (
            not isinstance(binding.trigger, ChordTrigger)
    ...
    def _processBinding(self, state, handData, timestampMs):
        ...
        if isinstance(t, ContinuousTrigger):
            ...
            value, ended = state.continuousTracker.update(result.pose, result.metrics, timestampMs, enabled=enabled)
        ...
        if isinstance(t, PoseTrigger):
            ...
            if stepsCompleted:
                self.publisher.sequenceProgress(b.name, t.hand, stepsCompleted, len(t.steps))
            if done:
                self.publisher.gestureFired(b.name, t.hand)
...
class GestureEngineService(dbus.service.Object):
    ...
    def GestureFired(self, name, hand): pass

    @dbus.service.signal(DBUS_IFACE, signature='ssd')
    def ContinuousUpdate(self, name, hand, value): pass

    @dbus.service.signal(DBUS_IFACE, signature='ss')
    def ContinuousEnd(self, name, hand): pass

    @dbus.service.signal(DBUS_IFACE, signature='ssii')
    def SequenceProgress(self, name, hand, step, total): pass
...
def renderDebugOverlay(frame, handData):
    ...
    if parts:
        cv2.putText(frame, "  ".join(parts), (10, 36),
...
def openCamera(camInput, width=None, height=None, fps=None):
...
def processFrame(frame, landmarker, processors, matcher, timestampMs):
    ...
    # Skip them — any real scene has a mean well above this threshold.
    if frame.mean() < 5.0:
        return
    ...
    handData = {}
    for side, processor in (("right", processors["right"]), ("left", processors["left"])):
        if side in handMap:
...
def main():
    ...
    parser.add_argument("--input",  default=None,            help="Camera index or path (overrides config camera setting)")
    parser.add_argument("--config", default=str(DEFAULT_CONFIG), help="Path to triggers.toml")
    parser.add_argument("--debug",  action="store_true",     help="Show OpenCV window with landmarks")
    ...
    cap = openCamera(camInput,
        width=settings.get("width"), height=settings.get("height"), fps=settings.get("fps"))
            ...
            while not stopFlag.is_set():
                if configWatcher.pollChanged():
                    try:
                        ...
                        processors     = {"right": HandProcessor(poses), "left": HandProcessor(poses)}
                        matcher        = TriggerMatcher(bindings, publisher, defaultDwellMs)
                        print(f"[config] Reloaded: {len(poses)} pose(s), {len(bindings)} binding(s)")
